...
但是，自然语言处理不仅仅需要关注每个单词的信息，还需要关注整个句子的信息，因此需要一个记录之前信息的记忆结构h（memory或state），在每个单词处理是都让它参与运算，这样不断累积，最后一个单词后得到的新h(新memory)就记录了整个句子的语义信息。![图片来源于互联网](/asset/2019-11-04/h.png)
